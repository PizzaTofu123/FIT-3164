...
module.exports = {
    ...
    deleteClubElection : async (req, res) => {
        try {
          ...
          // Call the query to delete the elections and update the club
          const updatedClub = await clubQueries.deleteClubElection(clubId);
      
          // If the club was not found, send a 404 response
          if (!updatedClub) {
            return res.status(404).json({ message: 'Club not found' });
          }
      
          // Send a success response after deleting the election
          res.status(200).json({ message: 'Election deleted successfully', club: updatedClub });
        } catch (error) {
