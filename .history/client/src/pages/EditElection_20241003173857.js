...
const EditElection = () => {
  ...
  const handleDeleteElection = async (e) => {
    ...
    console.log(`Attempting to delete election for club: ${clubName} with ID: ${clubId}`);
  
    if (window.confirm(`Are you sure you want to delete all elections for ${clubName}?`)) {
      try {
        // 1. Send request to delete all elections related to the club
        ...
        if (response.ok) {
          console.log(`Elections for ${clubName} deleted successfully.`);
          alert(`Elections for ${clubName} deleted successfully.`);
          // Redirect to the club representative page after successful deletion
          navigate('/clubrepresentative');  
        } else {
          console.error("Failed to delete elections:", response.statusText);
          alert("Failed to delete elections. Please try again.");
        }
      } catch (err) {
        console.error("Error occurred during election deletion:", err);
        alert("An error occurred while deleting the elections.");
      }
    }
  };
